...
load('Raw_images_numPos_180degree80.mat'); % generated images
load('./result80/Record80_02.mat');
% load('./result80_new_PN/Record80_harder06.mat');

%% Load the paras. for network
numPN = 360; numKC = 8000; numEN = 2; % archtecture
C_I_PN_sen = 0; C_I_PN_var = 3980; %1433-1435 works
...
% weight_matrix_PN_KC = connection_PN_KC.*(g_PN_KC);
weight_KC_EN = gpuArray(Record.weight_matrix_KC_EN); %load the trained weights

% record
KC_activity = gpuArray.zeros(numPos, numScan);
EN_activity = gpuArray.zeros(numPos, numScan);
numPos = 80; % set the number of positions want to testRaw_image
%% Testing phase (no learning)
reward = 0; interval = 15; %[ms]

for i_pos = 1:numPos, % for each position
    i_pos
    for i_off = 1:numDis+1, % for each off_loc image
        i_off
        if i_off == 1
            temp_img = Raw_images.position(i_pos).origin;
        else
            temp_img = Raw_images.position(i_pos).off_loc(i_off-1).raw_image;
        end
...
%         
        ...
        KC_activity(i_pos, i_off) = KC_count;
        EN_activity(i_pos, i_off) = EN_count;
    end    
end

%% Gather information and save
Record_off_response = struct;
Record_off_response.KC_activity = gather(KC_activity);
Record_off_response.EN_activity = gather(EN_activity);
% save('./result80/Record_scan_response_01.mat', 'Record_scan_response');
...
for i = 1:numPos,
    ...
    plot(1:(numDis+1), Record_off_response.EN_activity(i,:), 'rx-');
end
% saveas(figure(1), './result80_new_PN/80pos91scanImages06', 'jpg'); 
pause(5);
...
for i = 1:numPos,
    ...
    ylabel('EN spikes')
    subplot(2,1,2),
    plot(1:(numDis+1), Record_off_response.KC_activity(i,:), 'gd-');
    ylabel('KC spikes')
    hold off;
